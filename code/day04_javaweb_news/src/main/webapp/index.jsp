...
<div id="container">
  ...
  <div class="main">
    ...
    <div class="content">
      <ul class="class_date">
        <!-- 新闻主题链接区域 -->
        <c:forEach items="${allTopic}" var="news">
          <li><a href="/news?name=getnewsbytid"> ${news.tname}</a></li>
        </c:forEach>
      </ul>
      <ul class="classlist">

        <!-- 分页显示新闻区域 -->
<%--        <c:forEach items="${newsByPageNo}" var="news">--%>
<%--          <li class="newsItem"><a href="/news?name=getnewsbytid"> ${news.ntitle} <span>${news.ncreateDate}</span></a></li>--%>
<%--        </c:forEach>--%>
      </ul>
